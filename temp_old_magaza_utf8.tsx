...
const CATEGORIES = [
  { emoji: '­şÑ®', label: 'Mama & At─▒┼şt─▒rmal─▒k', count: 248, color: 'from-red-50 to-orange-50 border-orange-100 hover:border-orange-300' },
  { emoji: '­şĞ«', label: 'Tasma & Gezdirme', count: 134, color: 'from-blue-50 to-indigo-50 border-blue-100 hover:border-blue-300' },
  { emoji: '­şøÅ´©Å', label: 'Yatak & Yuva', count: 89, color: 'from-purple-50 to-violet-50 border-purple-100 hover:border-purple-300' },
  { emoji: '­şğ©', label: 'Oyuncak', count: 176, color: 'from-yellow-50 to-amber-50 border-yellow-100 hover:border-yellow-300' },
  { emoji: '­şÆè', label: 'Sa─şl─▒k & Vitamin', count: 92, color: 'from-green-50 to-emerald-50 border-green-100 hover:border-green-300' },
  { emoji: 'Ô£é´©Å', label: 'Bak─▒m & T─▒mar', count: 67, color: 'from-pink-50 to-rose-50 border-pink-100 hover:border-pink-300' },
];
...
// Vitrin hero ├╝r├╝nleri
const HERO_PRODUCTS = [
  { id: 1, name: 'Royal Canin', subtitle: 'Irka ├Âzel form├╝l', photo: 'https://images.unsplash.com/photo-1583337130417-3346a1be7dee?w=500&auto=format&fit=crop', discount: '%17', price: 289, color: 'from-amber-400 to-orange-500' },
  { id: 3, name: 'Ortopedik Yatak', subtitle: 'Eklem dostlar─▒ i├ğin', photo: 'https://images.unsplash.com/photo-1541781774459-bb2af2f05b55?w=500&auto=format&fit=crop', discount: '%15', price: 379, color: 'from-violet-500 to-purple-600' },
  { id: 7, name: 'Purina Pro Plan', subtitle: 'Kedi uzman─▒ mama', photo: 'https://images.unsplash.com/photo-1583337130417-3346a1be7dee?w=500&auto=format&fit=crop', discount: '%18', price: 319, color: 'from-teal-500 to-emerald-600' },
  { id: 9, name: 'Otomatik Su Kab─▒', subtitle: 'S├╝rekli taze su', photo: 'https://images.unsplash.com/photo-1541781774459-bb2af2f05b55?w=500&auto=format&fit=crop', discount: '%20', price: 450, color: 'from-blue-500 to-indigo-600' },
];
...
function ProductSlider() {
  ...
  return (
    <div className="relative bg-white/15 backdrop-blur-sm rounded-2xl overflow-hidden border border-white/20 h-full min-h-[160px]">
      <Link href={`/magaza/${product.id}`} className="p-5 h-full flex items-center gap-4 hover:bg-white/5 transition-colors cursor-pointer">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-xl font-bold text-white">Ôé║{product.price}</span>
            {product.oldPrice && <span className="text-xs line-through text-emerald-200">Ôé║{product.oldPrice}</span>}
          </div>
        </div>
      </Link>
      ...
      <div className="absolute top-3 right-3 text-[9px] font-bold bg-white/20 text-white px-2 py-0.5 rounded-full">├ûNE ├çIKAN</div>
    </div>
  );
}
...
export default function MagazaPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)]">
      ...
      <div className="bg-gradient-to-r from-emerald-600 to-teal-600 text-white py-10 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-3xl font-bold font-display">Pet Ma─şaza</h1>
              <p className="text-emerald-100 text-sm">Evcil dostlar─▒n i├ğin en iyi ├╝r├╝nler</p>
            </div>
          </div>
            ...
            <div className="flex-1 flex flex-col justify-center gap-4">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="├£r├╝n ara... (mama, tasma, oyuncak)"
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-2">
                {[{ icon: <Truck size={13}/>, label: 'Ôé║150 ├╝zeri ├╝cretsiz kargo' }, { icon: <Shield size={13}/>, label: 'G├╝venli al─▒┼şveri┼ş' }, { icon: <RefreshCw size={13}/>, label: '30 g├╝n iade garantisi' }].map((item, i) => (
                  <div key={i} className="flex items-center gap-1.5 text-xs text-emerald-100">{item.icon} {item.label}</div>
                ))}
              </div>
              <div className="flex flex-wrap gap-2">
                {['­şÑ® Mama', '­şĞ« Tasma', '­şğ© Oyuncak', '­şÆè Vitamin', 'Ô£é´©Å Bak─▒m'].map(chip => (
      ...
      {/* ÔöÇÔöÇ V─░TR─░N HERO ÔöÇÔöÇ */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <SectionHeader icon={<Zap size={18} className="text-white"/>} title="Vitrin ├£r├╝nleri" subtitle="Haftan─▒n en be─şenilen se├ğimleri" color="bg-gradient-to-br from-amber-400 to-orange-500" />
          ...
          {HERO_PRODUCTS.map(p => (
            <Link href={`/magaza/${p.id}`} key={p.id} className={`bg-gradient-to-br ${p.color} rounded-3xl p-6 text-white flex items-center gap-5 hover:scale-[1.02] transition-transform cursor-pointer shadow-lg overflow-hidden relative`}>
              ...
              <div className="flex-1 z-10">
                <div className="text-xs font-bold bg-white/25 text-white px-2 py-0.5 rounded-full inline-block mb-2">{p.discount} ─░ndirim</div>
                <div className="font-bold text-lg leading-tight">{p.name}</div>
                <div className="text-white/80 text-xs mt-0.5">{p.subtitle}</div>
                <div className="text-2xl font-bold mt-3">Ôé║{p.price}</div>
              </div>
            </Link>
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
          {CATEGORIES.map((cat, i) => (
            <button key={i} className={`flex flex-col items-center gap-2 p-4 bg-[var(--surface)] border rounded-2xl transition-all group ${cat.color}`}>
              ...
              <span className="text-[9px] text-[var(--foreground-muted)]">{cat.count} ├╝r├╝n</span>
            </button>
          ))}
        </div>
      </div>

      {/* ÔöÇÔöÇ ├çOK SATANLAR (2 Sat─▒r) ÔöÇÔöÇ */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        <SectionHeader icon={<Flame size={18} className="text-white"/>} title="├çok Satanlar" subtitle="Binlerce kullan─▒c─▒n─▒n tercihi" color="bg-gradient-to-br from-rose-500 to-red-600" />
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {BESTSELLERS.map(p => <ProductCard key={p.id} product={p} size="small" />)}
        </div>
      </div>

      {/* ÔöÇÔöÇ REKLAM ALANI ORTA ÔöÇÔöÇ */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        <AdBanner />
      </div>

      {/* ÔöÇÔöÇ ─░ND─░R─░MDEK─░LER ÔöÇÔöÇ */}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold font-display text-rose-700">─░ndirimdekiler</h2>
                <p className="text-xs text-rose-500">{ON_SALE.length} ├╝r├╝n indirimdeki f─▒rsatlar</p>
              </div>
            </div>
            <div className="flex items-center gap-1 text-xs text-rose-600 font-medium bg-rose-100 px-3 py-1.5 rounded-full">
              <Clock size={12}/> S─▒n─▒rl─▒ s├╝re
            </div>
          </div>
      ...
      {/* ÔöÇÔöÇ ├ûNE ├çIKAN ├£R├£NLER (2 Sat─▒r) ÔöÇÔöÇ */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <SectionHeader icon={<TrendingUp size={18} className="text-white"/>} title="├ûne ├ç─▒kan ├£r├╝nler" subtitle="Edit├Âr├╝n ├Ânerdi─şi se├ğimler" color="bg-gradient-to-br from-emerald-500 to-teal-600" />
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {FEATURED.map(p => <ProductCard key={p.id} product={p} size="small" />)}
        </div>
      </div>

      {/* ÔöÇÔöÇ T├£M ├£R├£NLER (5 Sat─▒r) ÔöÇÔöÇ */}
      <div className="bg-[var(--surface-secondary)] py-12 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold font-display">T├╝m ├£r├╝nler</h2>
            <span className="text-xs text-[var(--foreground-muted)]">{ALL_PRODUCTS.length} ├╝r├╝n</span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
      ...
      {/* ÔöÇÔöÇ FOOTER NOT ÔöÇÔöÇ */}
      <div className="py-8 bg-amber-50 border-y border-amber-200 mt-12 mb-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-sm font-semibold text-amber-800">
            Bu ├╝r├╝nler affiliate ortakl─▒k kapsam─▒nda ├Ânerilmektedir. sahiplendirme.com, do─şrudan ├╝r├╝n sat─▒┼ş─▒ yapmamaktad─▒r.
          </p>
        </div>
      </div>

      {/* ÔöÇÔöÇ REKLAM ALT ÔöÇÔöÇ */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        <AdBanner />
      </div>
    </div>
